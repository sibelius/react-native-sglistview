...
const PrivateMethods = {
  captureReferenceFor(cellData, sectionId, rowId, row) {
    console.log('captureReference: ', sectionId, rowId);
      ...
      if (changedRows.hasOwnProperty(section)) { // Good JS hygiene check
        ...
        for (const row in currentSection) {
          if (currentSection.hasOwnProperty(row)) { // Good JS hygiene check
            console.log('inside: ', section, row);
            if (typeof cellData[section] !== "undefined") {
              ...
              console.log('skip: ', section, row, cellData[section]);
            }
          }
        }
      }
  ...
  updateCellsPremptively(props, cellData, visibleRows) {
      ...
      if (visibleRows.hasOwnProperty(section)) { // Good JS hygiene check
        for (const row in visibleRows[section]) {
          ...
          if (typeof cellData[section] !== "undefined") {
            ...
            console.log('skip premptive: ', section, row);
          }
        }
      }
    }
    ...
    for (let i = 1; i <= props.premptiveLoading; i++) {
      ...
      if (isScrollingUp) {
        ...
        if (typeof cellData[firstVisibleRow] !== "undefined") {
          cell = cellData[firstVisibleSection][firstVisibleRow - i];
        } else {
          console.log('pree: first', lastVisibleRow);
        }
      } else if (isScrollingDown) {
        // cell = cellData[lastVisibleSection][lastVisibleRow + i];

        if (typeof cellData[lastVisibleRow] !== "undefined") {
          cell = cellData[lastVisibleSection][lastVisibleRow + i];
        } else {
          console.log('pree: last', lastVisibleRow);
        }
      }
...
const SGListView = React.createClass({
  ...
  onChangeVisibleRows(visibleRows, changedRows) {
    // console.log('onChangeVisibleRows v:', visibleRows);
    // console.log('onChangeVisibleRows c:', changedRows);
    // console.log('onChangeVisibleRows cell:', this.cellData);
